...
class Sm9EncKey(Structure):
	...
	def de_from_hex(self, hex_str):
		"""从hex字符串导入私钥de"""
		if not isinstance(hex_str, str):
			raise TypeError('hex_str must be string')
			
		# 清理输入字符串
		hex_bytes = hex_str.encode('utf-8')
		
		# 调用gmssl库函数转换点
		if gmssl.sm9_z256_twist_point_from_hex(byref(self.de), hex_bytes) != 1:
			raise ValueError('Invalid de hex string')
			
		print(f"[DEBUG] Import de: {hex_str}")
		self._has_private_key = True
  
	def ppub_from_hex(self, hex_str):
		"""从hex字符串导入公钥点Ppube"""
		if not isinstance(hex_str, str):
			raise TypeError('hex_str must be string')
		...
		# 调用gmssl库函数转换点
		if gmssl.sm9_z256_point_from_hex(byref(self.Ppube), hex_bytes) != 1:
			raise ValueError('Invalid Ppube hex string')
			
		print(f"[DEBUG] Import Ppube: {hex_str}")
	...
	def decrypt(self, ciphertext):
		if self._has_private_key != True:
			raise TypeError('has no private key')
		# 计算明文长度
		plaintext_size = len(ciphertext) - 96  # C1(64) + C3(32) = 96
		# plaintext_size = SM9_MAX_PLAINTEXT_SIZE
		# 动态分配明文缓冲区
		plaintext = create_string_buffer(plaintext_size)
		outlen = c_size_t()
		if gmssl.sm9_decrypt_ship_id(byref(self), c_char_p(self._id), c_size_t(len(self._id)),
			ciphertext, c_size_t(len(ciphertext)), plaintext, byref(outlen)) != 1:
			raise NativeError('libgmssl inner error')
		return plaintext[0:outlen.value]

	def print_key(self, label="SM9 Encryption Key"):
		"""打印SM9加密密钥信息"""
		# 打开stdout
		libc.fdopen.restype = c_void_p
		stdout = libc.fdopen(1, "w".encode('utf-8'))
  
		# 转换label为bytes
		label_bytes = label.encode('utf-8')
  
		# 调用gmssl打印函数
		if gmssl.sm9_enc_key_print(
      		c_void_p(stdout),  # FILE指针
        	1,                 # 格式化输出
			4,                 # 缩进
			c_char_p(label_bytes),  # 标签
			byref(self)       # 密钥结构体	
   			) != 1:
      			raise NativeError('打印密钥失败')
...
class Sm9EncMasterKey(Structure):
	...
	def ppub_from_hex(self, hex_str):
		...
		if gmssl.sm9_z256_point_from_hex(byref(self.Ppube), hex_str) != 1:
			raise NativeError('libgmssl inner error')
		self._has_public_key = True
  
	def ke_from_hex(self, hex_str):
		"""从hex字符串导入主私钥ke"""
		if not isinstance(hex_str, str):
			raise TypeError('hex_str must be string')
		if len(hex_str) != 64:
			raise ValueError('Invalid ke hex string length')
		hex_str = hex_str.encode('utf-8')
		if gmssl.sm9_z256_from_hex(byref(self.ke), hex_str) != 1:
			raise ValueError('Invalid ke hex string')
		print("ke_from_hex: ", hex_str)
		self._has_private_key = True
	...
	def encrypt(self, plaintext, to):
		if self._has_public_key != True:
			raise TypeError('has no public master key')
		to = to.encode('utf-8')
		# 计算密文长度: C1(64) + C3(32) + C2(明文长度)
		ciphertext_size = 64 + 32 + len(plaintext)
		# ciphertext_size = SM9_MAX_CIPHERTEXT_SIZE
		# 动态分配缓冲区
		ciphertext = create_string_buffer(ciphertext_size)
		outlen = c_size_t()
		if gmssl.sm9_encrypt_ship_id(byref(self), c_char_p(to), c_size_t(len(to)),
			plaintext, c_size_t(len(plaintext)), ciphertext, byref(outlen)) != 1:
			raise NativeError('libgmssl inner error')
		return ciphertext[0:outlen.value]

	def print_key(self, label="SM9 Encryption Master Key"):
		"""打印SM9主密钥信息"""
		# 打开stdout
		# 查看 8 个 64 位分段（十进制）
		segments_dec = [self.ke.d[i] for i in range(4)]
		print("64-bit segments in decimal:", segments_dec)

		# 查看 8 个 64 位分段（十六进制）
		segments_hex = [hex(self.ke.d[i]) for i in range(4)]
		print("64-bit segments in hex:", segments_hex)
		...
		# 调用gmssl打印函数
		if gmssl.sm9_enc_master_key_print(
			c_void_p(stdout),  # FILE指针
			1,                 # 格式化输出
			4,                 # 缩进
			c_char_p(label_bytes),  # 标签
...
class Sm9SignKey(Structure):
	...
	def ppubs_from_hex(self, hex_str):
		"""从hex字符串导入公钥点Ppubs"""
		if not isinstance(hex_str, str):
			raise TypeError('hex_str must be string')
		...
		if gmssl.sm9_z256_twist_point_from_hex(byref(self.Ppubs), hex_bytes) != 1:
			raise ValueError('Invalid Ppubs hex string')
			
		print(f"[DEBUG] Import Ppubs: {hex_str}")
  
	def ds_from_hex(self, hex_str):
		"""从hex字符串导入私钥ds"""
		if not isinstance(hex_str, str):
			raise TypeError('hex_str must be string')
		...
		if gmssl.sm9_z256_point_from_hex(byref(self.ds), hex_bytes) != 1:
			raise ValueError('Invalid ds hex string')
			
		print(f"[DEBUG] Import ds: {hex_str}")
		self._has_private_key = True
	...
	def print_key(self, label="SM9 Signature Key"):
		"""打印SM9签名密钥"""
		libc.fdopen.restype = c_void_p
		stdout = libc.fdopen(1, "w".encode('utf-8'))
		label_bytes = label.encode('utf-8')
		
		if gmssl.sm9_sign_key_print(
			c_void_p(stdout),
			1,
			4,
			c_char_p(label_bytes),
			byref(self)
		) != 1:
			raise NativeError('打印密钥失败')
...
class Sm9SignMasterKey(Structure):
	...
	def ks_from_hex(self, hex_str):
		"""从hex字符串导入主私钥ks"""
		if not isinstance(hex_str, str):
			raise TypeError('hex_str must be string')
			
		if len(hex_str) != 64:
			raise ValueError('Invalid ks hex length')
			
		hex_bytes = hex_str.encode('utf-8')
		if gmssl.sm9_z256_from_hex(byref(self.ks), hex_bytes) != 1:
			raise ValueError('Invalid ks hex string')
			
		print(f"[DEBUG] Import ks: {hex_str}")
		self._has_private_key = True
	...
	def print_key(self, label="SM9 Signature Master Key"):
		"""打印SM9签名主密钥"""
		libc.fdopen.restype = c_void_p
		stdout = libc.fdopen(1, "w".encode('utf-8'))
		label_bytes = label.encode('utf-8')
		
		if gmssl.sm9_sign_master_key_print(
			c_void_p(stdout),
			1,
			4,
			c_char_p(label_bytes),
...
class Sm9Signature(Structure):
	...
	def sign(self, sign_key):
		...
		siglen = c_size_t()
		if gmssl.sm9_sign_finish_ship_id(byref(self), byref(sign_key), sig, byref(siglen)) != 1:
			raise NativeError('libgmssl inner error')
		return sig[:siglen.value]
	...
	def verify(self, signature, public_master_key, signer_id):
		...
		if gmssl.sm9_verify_finish_ship_id(byref(self), signature, c_size_t(len(signature)),
