...
// ship_id
int test_sm9_z256_sign_ship_id()
{
    SM9_SIGN_CTX ctx;
    SM9_SIGN_KEY key;
    SM9_SIGN_MASTER_KEY mpk;
    ...
    uint8_t data[20] = {0x43, 0x68, 0x69, 0x6E, 0x65, 0x73, 0x65, 0x20, 
                        0x49, 0x42, 0x53, 0x20, 0x73, 0x74, 0x61, 0x6E, 
                        0x64, 0x61, 0x72, 0x64};
    uint8_t IDA[5] = {0x41, 0x6C, 0x69, 0x63, 0x65};

    // 主密钥生成
    sm9_z256_from_hex(mpk.ks, hex_ks);
    sm9_z256_twist_point_mul_generator(&(mpk.Ppubs), mpk.ks);
    
    // 密钥提取
    if (sm9_sign_master_key_extract_key(&mpk, (char *)IDA, sizeof(IDA), &key) < 0) 
        goto err; 
    ++j;

    // 验证密钥
    sm9_z256_point_from_hex(&ds, hex_ds);
    if (!sm9_z256_point_equ(&(key.ds), &ds)) 
        goto err; 
    ++j;
    ...
    sm9_sign_init(&ctx);
    sm9_sign_update(&ctx, data, sizeof(data));
    if (sm9_sign_finish_ship_id(&ctx, &key, sig, &siglen) < 0) 
        goto err; 
    ++j;
    ...
    sm9_verify_init(&ctx);
    sm9_verify_update(&ctx, data, sizeof(data));
    if (sm9_verify_finish_ship_id(&ctx, sig, siglen, &mpk, 
        (char *)IDA, sizeof(IDA)) != 1) 
        goto err; 
    ++j;
...
// ship_id
// int test_sm9_z256_encrypt_ship_id()
// {
//     SM9_ENC_MASTER_KEY msk;
//     SM9_ENC_KEY key;
//     SM9_Z256_TWIST_POINT de;
...
//     if (sm9_encrypt_ship_id(&msk, (char *)IDB, sizeof(IDB), data, sizeof(data), out, &outlen) < 0) goto err; ++j;
//     if (sm9_decrypt_ship_id(&key, (char *)IDB, sizeof(IDB), out, outlen, dec, &declen) < 0) goto err; ++j;
//     if (memcmp(data, dec, sizeof(data)) != 0) goto err; ++j;
...
int test_sm9_z256_encrypt_ship_id()
{
    ...
    // 初始化测试数据
    memset(data, 0x66, sizeof(data));  // 用0x66填充测试数据
    
    // 分配解密缓冲区
    if (!(dec = malloc(declen)) || 
        !(out = malloc(1000 + 64 + SM3_HMAC_SIZE))) { // C1(64) + C3(32) + C2(1000)
        goto err;
    }
    ...
    if (sm9_enc_master_key_extract_key(&msk, (char *)IDB, sizeof(IDB), &key) < 0) 
        goto err; 
    ++j;

    sm9_z256_twist_point_from_hex(&de, hex_de); 
    if (!sm9_z256_twist_point_equ(&(key.de), &de)) 
        goto err; 
    ++j;

    if (sm9_encrypt_ship_id(&msk, (char *)IDB, sizeof(IDB), 
        data, sizeof(data), out, &outlen) < 0) 
        goto err; 
    ++j;

    if (sm9_decrypt_ship_id(&key, (char *)IDB, sizeof(IDB), 
        out, outlen, dec, &declen) < 0) 
        goto err; 
    ++j;

    if (memcmp(data, dec, sizeof(data)) != 0) 
        goto err; 
    ++j;
...
int main(void) {
	...
	if (test_sm9_z256_sign_ship_id() != 1) goto err;
	if (test_sm9_z256_ciphertext() != 1) goto err;
	if (test_sm9_z256_encrypt() != 1) goto err;
    if (test_sm9_z256_encrypt_ship_id() != 1) goto err;
