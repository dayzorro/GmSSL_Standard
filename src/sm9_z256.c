...
// ship_id
int sm9_z256_point_to_uncompressed_octets_ship_id(const SM9_Z256_POINT *P, uint8_t octets[64])
{
    sm9_z256_t x;
    sm9_z256_t y;
    ...
    octets[0] = 0x04;
    sm9_z256_to_bytes(x, octets);
    sm9_z256_to_bytes(y, octets + 32);
    return 1;
}
...
// ship_id
int sm9_z256_point_from_uncompressed_octets_ship_id(SM9_Z256_POINT *P, const uint8_t octets[64])
{
    sm9_z256_from_bytes(P->X, octets);
    if (sm9_z256_cmp(P->X, SM9_Z256_P) >= 0) {
        error_print();
        return -1;
    }
    ...
    sm9_z256_from_bytes(P->Y, octets + 32);
    if (sm9_z256_cmp(P->Y, SM9_Z256_P) >= 0) {
        error_print();
        return -1;
    }
