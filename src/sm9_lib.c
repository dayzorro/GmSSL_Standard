                   ...
                   uint8_t *out)
{
    ...
    uint8_t k[SM9_MAX_PLAINTEXT_SIZE + SM3_HMAC_SIZE];
    uint8_t mac[SM3_HMAC_SIZE];
    size_t klen = c2len + SM3_HMAC_SIZE;
...
int sm9_decrypt_ship_id(const SM9_ENC_KEY *key, const char *id, size_t idlen,
                const uint8_t *in, size_t inlen, uint8_t *out, size_t *outlen)
{
    // 基础长度检查：至少要能容纳 C1 和 C3
    const size_t C1_LEN = 64;
    if (inlen < (C1_LEN + SM3_HMAC_SIZE)) {
        return -1;
    }

    // 分别定位 C1, C3, C2
    const uint8_t *c1 = in;                           // C1 的起始地址
    const uint8_t *c3 = c1 + C1_LEN;                  // C1 后面紧接的 C3
    const uint8_t *c2 = c3 + SM3_HMAC_SIZE;           // C3 之后就是 C2
    size_t c2len = inlen - (C1_LEN + SM3_HMAC_SIZE);  // C2 的长度

    // 将 C1 转回椭圆曲线点
    SM9_Z256_POINT C1;
    if (sm9_z256_point_from_uncompressed_octets_ship_id(&C1, c1) != 1) {
        error_print();
        return -1;
    }
