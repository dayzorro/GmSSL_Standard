...
// ship_id
int sm9_signature_to_ship_id(const SM9_SIGNATURE *sig, uint8_t **out, size_t *outlen)
{
	// 计算所需长度
    size_t total_len = 32 + 65;
    *outlen = total_len;  // 先设置输出长度

    uint8_t hbuf[32];
    uint8_t Sbuf[65];
    // 转换h和S到字节格式
    sm9_z256_to_bytes(sig->h, hbuf);
    if (sm9_z256_point_to_uncompressed_octets(&sig->S, Sbuf) != 1) {
        error_print();
        return -1;
    }
...
// ship_id
int sm9_signature_from_ship_id(SM9_SIGNATURE *sig, const uint8_t **in, size_t *inlen)
{
    const uint8_t *d = *in;
    size_t dlen = *inlen;
    
    // 检查长度
    if (dlen < 97) {  // 32(h) + 65(S)
        error_print();
        return -1;
    }
...
// ship_id
int sm9_sign_finish_ship_id(SM9_SIGN_CTX *ctx, const SM9_SIGN_KEY *key, uint8_t *sig, size_t *siglen)
{
	SM9_SIGNATURE signature;
	...
	*siglen = 0;
	if (sm9_signature_to_ship_id(&signature, &sig, siglen) != 1) {
		error_print();
		return -1;
	}
	return 1;
...
// ship_id
int sm9_verify_finish_ship_id(SM9_SIGN_CTX *ctx, const uint8_t *sig, size_t siglen,
    const SM9_SIGN_MASTER_KEY *mpk, const char *id, size_t idlen)
{
    int ret;
    SM9_SIGNATURE signature;

    if (sm9_signature_from_ship_id(&signature, &sig, &siglen) != 1
        || siglen != 0) {  // 检查是否完全解析
        error_print();
        return -1;
    }
